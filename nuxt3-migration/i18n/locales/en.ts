export default defineI18nLocale(async () => ({
  common: {
    title: "KamiMap",
    site_name: "KamiMap - Paper Map",
    site_desc: "Paper Map for printable map information",
    about: "About",
    contribute: "Contribute",
    share: "Share",
    language: "Language",
    printed_at: "Printed at:",
    back_to_home: "Back to Home"
  },
  map: {
    desc_1: "Smartphone battery is precious during an earthquake. Let's have a paper map.",
    desc_2: "The maps on this site are designed to be printed and used.",
    desc_3: "Paper maps can be used even when your smartphone runs out of charge.",
    desc_4: "Please print this map for evacuation and support.",
    desc_5: "Press the \"Print\" button in the upper right to display the print page.",
    desc_6: "After adjusting the scale and display area on the map, you can print a clear map.",
    desc_7: "If you are viewing on a tablet or smartphone, scroll the screen to see a detailed information list below the map area."
  },
  PrintableMap: {
    name: "Name:",
    legend: "Legend",
    show_all: "Show All",
    print: "Print",
    back_to_map: "Back to Map",
    close_area_select: "Close Area Selection",
    close_list: "Close List",
    no_point_in_map: "There is no point information in this map area."
  }
}
))